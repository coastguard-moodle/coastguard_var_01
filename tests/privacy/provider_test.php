...
/**
 * Privacy tests for theme_coastguard_var_01.
 *
 * @package    theme_coastguard_var_01
 * @category   test
 * @covers     \theme_coastguard_var_01\privacy\provider
...
class provider_test extends \core_privacy\tests\provider_testcase {
    ...
    public function test_export_user_preferences(string $preference, bool $value, string $expectdescription): void {
        ...
        $exportedpreferences = $writer->get_user_preferences('theme_coastguard_var_01');
        $this->assertCount(1, (array) $exportedpreferences);
        $this->assertEquals($value, (bool) $exportedpreferences->{$preference}->value);
        $this->assertEquals(get_string($expectdescription, 'theme_coastguard_var_01'), $exportedpreferences->{$preference}->description);
    }
}
